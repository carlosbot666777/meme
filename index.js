...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Vai com deus e pomba do divindo @${num.split('@')[0]}👋😎`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				esperar: '⌛ Em processo bafo de pênis⌛',
				sucesso: '✔️ Pronto fdp✔️',
				erro: {' DEU MERDA!😔,'
					grudar: '❌ Falha, ocorreu um erro ao converter a imagem em um adesivo ❌',
					Iv: '❌ Link inválido ❌'
				},
				only: {
...
case 'ADM GOSTOSO':
					...
					buffer = await getBuffer(`https://i.imgur.com/HsXzwQU.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'CARLOS 😏'})
					break
				/*case 'memeindo':
					memein = await kagApi.memeindo()
				...
				case 'bc':
					...
					if (mentioned.length > 1) {
						teks = 'AGORA VOCÊ É ADM😎\n'
						for (let _ de mencionado) {
							teks += `@${_.split('@')[0]}\n`
						}
